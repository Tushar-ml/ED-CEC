...
def create_rare_word_list(path, max_count: int = 15):

    json_data = json.load(open(path, "r"))
    words_count_data = defaultdict(int)
    ...
    for text in json_data.values():
        ...
        for word in words:
            if not word.isnumeric():
                words_count_data[word] += 1

    sorted_dict = sorted(words_count_data.items(), key=lambda item: item[1])
    sorted_words = [k for k,v in sorted_dict if v <= max_count]

    rare_output_txt = path.replace(".json", "_rare.txt")
    with open(rare_output_txt, "w") as f:
