...
# Function to tokenize and obtain hidden representations of contextual items
def encode_contextual_items(contextual_items,model_path, max_length=512) -> torch.Tensor:
    tokenizer = BertTokenizer.from_pretrained(model_path)
    model = BertModel.from_pretrained(model_path)

    # Tokenize contextual items
    tokenized_contextual_items = tokenizer(contextual_items, max_length=max_length, padding=True, truncation=True, return_tensors='pt')

    # Obtain hidden representations using the BERT model
    with torch.no_grad():
        output = model(**tokenized_contextual_items)
...
class ContextDecoder(nn.Module):
    def __init__(self, args) -> None:
        ...
        self.args = args
        rare_word_list = args.rare_words_list

        words_list = [line for line in open(rare_word_list, 'r').readlines()]
        self.contextual_encoder = encode_contextual_items(words_list,args.base_model).to(args.device)
        self.m_o = nn.Parameter(torch.randn(1, 768))

        self.decoder_proj = nn.Sequential(nn.Linear(args.tag_hidden_size, args.vocab_size), \
                                          nn.Softmax(dim=-1))
        
        self.weighted_proj = nn.LazyLinear(1)
        self.device = args.device

    def forward(self, gen_outputs):

        mean_c = self.contextual_encoder.mean(dim=1)
        m_c = torch.concatenate([self.m_o, mean_c])
        gen_output = gen_outputs.to(self.device)
        scores = torch.matmul(gen_output, m_c.t())
        gate_t = scores[:, 0]
        g = torch.sigmoid(self.weighted_proj(gate_t))

        argmax = torch.argmax(gate_t).item()
        ...
        if argmax != 0:
            ...
            root_d = 1
            o_con = F.softmax(torch.matmul(gen_output, e_m_c.t())/root_d)
...
class TagDecoder(nn.Module):
    def __init__(self, args):
        ...
        self.edit_decoder = nn.TransformerDecoder(decoder_layer, num_layers=12)
        self.decoder_proj = nn.Sequential(nn.Linear(args.tag_hidden_size, args.vocab_size), \
                                          nn.Softmax(dim=-1))
        ...
        self.contextual_decoder = ContextDecoder(args)
    ...
    def forward(self, inputs):
        ...
        if target.size(-1) == 1:
            ...
            con_probs, g, scores, to_consider_context = self.contextual_decoder(decoder_outputs)
            contextual_loss = torch.tensor([0]).to(self.args.device)
            gen_probs = self.decoder_proj(decoder_outputs)

            if to_consider_context:
                g = g.unsqueeze(-1)
        ...
        total_loss = self.args.alpha * tag_loss + gen_loss + contextual_loss
        return tag_loss, gen_loss,contextual_loss, total_loss
