...
def train_epoch(model, train_data_loader, optimizer):
    ...
    for i, batch in enumerate(train_data_loader):
        optimizer.zero_grad()
        tag_loss, gen_loss, contextual_loss, total_loss = model(batch)
        total_loss.backward()
        ...
        print(f"train step: {i}, tag loss is {tag_loss.item()}, gen loss is {gen_loss.item()}, context loss is {contextual_loss.item()} total loss is {total_loss.item()}")
...
if __name__ == '__main__':
    ...
    parser.add_argument("--rare_words_list", type=str, default="", help="")
